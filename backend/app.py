...
def func(wallet):
  ...
  for movie in user["data"][m:n]:
    ...
    if index >= 0:
      distances = sorted(list(enumerate(similarity[index])),
                         reverse=True,
                         key=lambda x: x[1])
      for gh in distances[1:15]:
        poster_path = fetch_poster(new['movie_id'][gh[0]])
        if poster_path is None:
          break
        recomendations["titles"].append(new['title'][gh[0]])
        recomendations["posters_url"].append(
  ...
  for x in unique_list:
    li.append(recomendations["posters_url"][recomendations["titles"].index(x)])
  recomendations["posters_url"] = li
  recomendations["titles"] = unique_list
  return jsonify(recomendations)
